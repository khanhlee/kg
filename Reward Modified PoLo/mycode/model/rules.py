...
def modify_rewards(rule_list, arguments, query_rel_string, obj_string, rewards, only_body, positive_reward, theta1, theta2, alpha):
    rule_count = 0
    rule_count_body = 0
        ...
        if query_rel in rule_list:
            ...
            for j in range(len(rel_rules)):
                if check_rule(body, obj, obj_string[k], rel_rules[j], only_body):
                    add_reward = positive_reward * (theta1 + ((theta2 - theta1)/(1+np.exp(-20*(float(rel_rules[j][0])-alpha)))))
